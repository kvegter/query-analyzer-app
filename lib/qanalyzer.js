...
class QueryAnalyzer {
    ...
    async init() {
        let prms;
        console.log("init db functionality");
        ...
        if (nac.neoConnection && nac.neoConnection == true) {
            ...
            if (nac.neodb.apocAvailable && nac.neodb.apocAvailable == true) {
    ...
    _buildQSettingsScreen() {
        ...
        let html = "<table class=\"ui celled table\">\n" +
            ...
            "  <tbody>\n" ;
        // queryLog
        if (this.logConfig.queryLog.value === "true " || this.logConfig.queryLog.value === "false") {
            // < version 4
        } else {
        ...
        html +=  "<tr>\n" +
            ...
            "      <td data-label=\"Value\">" + this.logConfig.queryLog.value + "</td>\n" +
            "      <td data-label=\"Description\">" + this.logConfig.queryLog.description + "</td>\n" +
            "    </tr>\n" ;
        // queryTreshold
        html +=  "<tr>\n" +
            "      <td data-label=\"Setting\">Query Threshold</td>\n" +
            "      <td data-label=\"Value\">" + this.logConfig.queryThreshold.value + "</td>\n" +
            "      <td data-label=\"Description\">" + this.logConfig.queryThreshold.description + "</td>\n" +
            "    </tr>\n" ;
    ...
    _createCheckbox(anId, aValue) {
     ...
     _QueryLogSettingsInfo(aresult) {
        ...
        aresult.records.forEach(function(record) {
            ...
            let dynamic = record.get("dynamic");
    ...
    async analyseLogStream() {
        ...
        if (nac.neoConnection && nac.neoConnection == true && nac.neodb.apocAvailable == true) {
            ...
            let qry = "call apoc.log.stream('query.log') yield lineNo, line return line order by lineNo asc";
            let session = nac.getReadSession(); // without a name it will be the default database
            let rs = await nac.runQuery(session, qry,{});
            let lgLines = [];
...
//        let hlcolor = "#919A78"; #00ff00, #63B345
    ...
    _parseBoltLineV3(aRecord) {
        ...
        //console.log(" field0fields " + field0fields);
        let baseindexje = 3;
