...
class QueryAnalyzer {
    ...
    _prepareSummarytab() {
        ...
        let html = "<div id='"+ this.SUMMARY_FILTER_KEY+ "' class='ui fluid left icon input'><input  id='"+ this.FILTER_INPUT_KEY + "' type='text' placeholder='Hit enter to Filter the queries containing this text. When less than 3 character are entered all the distinct queries will be shown. '/><i class='filter icon'></i></div>";
    ...
    onFilterChange(event) {
        ...
        if (this._isEnter(event)) {
            ...
            if (filter.length > 2 || this.lastFilter.length > 2)  {
    ...
    _onFilterChangeImpl() {
        ...
        let summaryFilter = document.getElementById(this.SUMMARY_FILTER_KEY);
        if (filter.length > 2 ) {
            ...
            if (filter.length < this.lastFilter.length ) {
    ...
    _buildQSettingsScreenCluster() {
        ...
        for (let key of this.configMap.keys()) {
            ...
            if (config.queryLog.dynamic == false || nac.neodb.isAura) {
                ...
                html += this._createCheckbox('showRuntime@' + key, config.runtimeLog.value);
            }
            html += "</td>\n";
        }
    ...
    async _saveQueryLogSettingsImpl(aKey, aDriver) {
        ...
        if (config.runtimeLog) {
            let showRuntime ="" +  document.getElementById("showRuntime" + key).checked;
            if (config.runtimeLog.value != showRuntime) {
                updateStatements.push("call dbms.setConfigValue('"+ config.runtimeLog.property+"','"+ showRuntime +"')");
            }
        }
        // cpuTrack
        ...
        // timeLog
        let timeLog = "" + document.getElementById("timeLog" + key).checked;

        if (config.timeLog.value != timeLog) {
            updateStatements.push("call dbms.setConfigValue('"+ config.timeLog.property+"','"+ timeLog +"')");
        }
        // allocationTrack
        let allocationTrack = "" + document.getElementById("allocationTrack" + key).checked;
        ...
        // allocationLog
        let allocationLog = "" + document.getElementById("allocationLog" + key).checked;
    ...
    analyseFile(aFileString) {
        ...
        let ht = "<table width='100%'><tr><td width='520px'>" + (this.log.length) + " finished queries analysed, " + this.ana.length+ " distinct queries found.<br/> queries from " + (new Date(this.minLogTime)).toUTCString() + " to " + (new Date(this.maxLogTime)).toUTCString() + "</td>";
        ...
        if (nac.neoConnection && nac.neoConnection == true && nac.neodb.apocAvailable == true) {
            ...
            try {
                if (nac.driverMap && nac.driverMap.size > 0) {
                    ...
                    let session = null;
                    if (nac.neodb.defaultDatabase && na.neodb.defaultDatabase != '') {
    ...
    _initQueryLogImpl(withFilters){
        ...
        html += "<button class='compact ui mini button' onClick='qan._showLogLoading()'>Show Query Log</button> Note this can take some time when there a lot queries in the log";
        html +="</div>";

        if (withFilters && withFilters == true) {
            // onkeyup='qan.onLogFilterChange();'
            html += "<div id='" + this.LOG_FILTER_SECTION_KEY + "'>"
            html += "<table width='100%' style='border-spacing: 0px; padding: 0px;'><tr><td style='width: 280px'><div id='"+ this.LOG_DATE_FILTER_KEY+ "' class='ui fluid left icon input'><input id='"+ this.LOG_DATE_FILTER_INPUT + "' type='text' placeholder='Hit Enter to filter on log date.'/><i class='filter icon'></i></div></div>";
    ...
    _analyseLog(logLines) {
        ...
        for (let ri = 0; ri < logLines.length; ri++ ) {
            ...
            // skip this entry when starting with //
            ...
            if ( isNewEntry == true || isLastRow == true) {
                ...
                //
                if (record.length == 9 || record.length == 8 || record.length == 5 || record.length == 3 || record.length == 7 || record.length > 9 ) {
                   ...
                   //  console.log(qrec);
                    ...
                    if (qrec.startOnly && qrec.startOnly == true) {
    ...
    _buildSummary() {
        ...
        for (let i = 0 ; i < anas.length; i++) {
            ...
            if (dta.count == 1) {
                ...
                contents += "<td colspan='8'><div style='font-family: monospace ; font-size: .9em; width: 1000px; word-wrap : break-word'>" + this._formatCypher(dta.query) + "</div></td>";
    ...
    onLogFilterChange(event) {
        ...
        if (this._isEnter(event)) {
            if (dateFilter.length > 2 || queryFilter.length > 2 || this.lastLogFilter.length > 2)  {
                if (this.log.length > 0) {
                    logTableSection.style = "";
            ...
            if (dateFilter.length > 2 || queryFilter.length > 2) {
                logTableSection.style="opacity: 0.5;";
            }
        }
    }
    __onLogFilterChangeImpl() {
        ...
        if (dateFilter.length > 2 || queryFilter.length > 2) {
            ...
            if (dateFilter.length >= queryFilter.length) {
                this.lastLogFilter = dateFilter;
            } else {
                this.lastLogFilter = queryFilter;
            }
        } else {
            // clear the filter only if lastLogFilter > 2
            if (this.lastLogFilter.length > 2) {
                let msg = "Loading queries ";
                if (dateFilter.length < 3) {
                    dateFilterInput.style = 'background-color: ' + this.filterInActiveBackground_color;
                } else {
                    msg += " log date filter:'" + dateFilter + "'";
                }
                if (queryFilter.length < 3) {
                    queryFilterInput.style = 'background-color: ' + this.filterInActiveBackground_color;
                } else {
                    msg += " query filter:'" + queryFilter + "'";
                }
                if (dateFilter.length < 3 && queryFilter.length < 3) {
                    msg = "Loading all " + this.log.length + " queries";
                }
    ...
    _loadQueryLog(loadingmsg) {
        ...
        let logTableSection = document.getElementById(this.LOG_TABLE_SECTION_KEY);
        logTableSection.innerHTML = "<div class=\"ui segment\">\n" +
            "  <div class=\"ui active inverted dimmer\">\n" +
            "    <div class=\"ui text loader\">" + lmsg + "</div>\n" +
            "  </div>\n" +
            "  <p><br/><br/><br/><br/><br/><br/><br/><br/><br/><br/><br/><br/><br/><br/><br/><br/><br/><br/><br/><br/><br/><br/><br/><br/><br/><br/><br/><br/><br/><br/><br/><br/><br/><br/><br/><br/><br/></p>\n" +
    ...
    _buildQueryLog(index, pShowAll){
        ...
        header += "<div class='ui message' style='width: 15950px;'><div class='header'><button class=\"ui icon button\" onclick=\"qan.clearAppLogFile()\"><i class=\"eraser icon\"></i></button>";
        if (applyFilter) {
            let dbstr = "";
        ...
        for (let i = 0; i < this.log.length; i++) {
            ...
            contents += "<td><div style='width: 300px; word-wrap : normal'>" + sparams + "</div></td>";
        ...
        tableSection.innerHTML = contents;
        return true;
