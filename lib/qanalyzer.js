...
class QueryAnalyzer {
    ...
    constructor() {
        ...
        //this.SUMMARY_SLIDER = "__summarySlider";
        this.SUMMARY_TRESHOLD = "__summaryTrheshold";
    ...
    async init() {
        ...
        document.getElementById(this.SUMMARY_TRESHOLD).addEventListener("change", function(event) {
            // tricky if we use 'current' in the constructor than you get the 'current' snapshot in time for 'this' which is not completely filled
            // Putting it here than it is save
            current.onThresholdChange(event);
        });
    ...
    _prepareSummarytab() {
        ...
        //let html = '<div id="' + this.SUMMARY_SLIDER + '" style="width: 100%;">date slider here</div><div class="ui input" style="width: 10%;"><input id="' + this.SUMMARY_TRESHOLD + '" type="number" placeholder="Enter threshold (ms)" step="10" min="0"/></div>';
        let html = '<div class="ui input" style="width: 10%;"><input id="' + this.SUMMARY_TRESHOLD + '" type="number" placeholder="Enter threshold (ms)" step="10" min="0"/></div>';
        html +=  "<div id='"+ this.SUMMARY_FILTER_KEY+ "' class='ui left icon input' style='width: 90%;'><input  id='"+ this.FILTER_INPUT_KEY + "' type='text' placeholder='Hit enter to Filter the queries containing this text. When empty all the distinct queries will be shown. Use db:<dbname>[@<serverid>] to filter on database and server. Example \"db:neo4j@2 caches\".'/><i class='filter icon'></i></div>";
    ...
    onThresholdChange(event) {
        let treshold = document.getElementById(this.SUMMARY_TRESHOLD).value;
        if (treshold === undefined) {
            treshold = 0;
        }
        this._showFilteredSummary("Loading using filter: " + treshold );
    }
    ...
    __analysisResults() {
        ...
        ht += "<br/> queries from " + this._formatDate(this.minLogTime,false) + " to " + this._formatDate(this.maxLogTime, false) + "</td>";
        return ht;
    }
    _clearLogRelatedViews() {
        ...
        if (document.getElementById(this.UNFINISHED_QUERY_HEADER)) {
            document.getElementById(this.UNFINISHED_QUERY_HEADER).innerHTML = "<div class='ui fluid segment'>\n            In this tab we show the queries which are started but not finished at the moment the log is copied and uploaded or streamed. This functionality is available in Neo4j version 4+ and query logging in VERBOSE mode. \n        </div>";
        }
    }
    ...
    __analyseLogEntry(curLine, srv, map) {
        // let check = curLine.indexOf('id:90043');
        // if ( check > -1) console.log(curLine);
        let proceed = true;
        let record = [];
        let isJSON = false;
        ...
        if (isJSON === true || isAuraJSON === true ||record.length == 9 || record.length == 8 || record.length == 5 || record.length == 3 || record.length == 7 || record.length > 9 ) {
            ...
            // if (check > -1 ) console.log(qrec)
            if (qrec === null) {
                return false; // skipping the explain records
            }
            qrec.srv = srv;
            if (qrec.startOnly === undefined) {
                console.log("NO START ONLY FOUND");
            }
    ...
    onSliderChange(params) {
        ...
        return { fromDisplay: params.from, toDisplay:  params.to } ;
    }
    _buildSummary() {
        let anas = this.ana;
        let contents = "";
        ...
        // slider
        //
        // let slider = new TwoHandleRangeSlider(this.SUMMARY_SLIDER,{}, this);
        // slider.load();
        //$('input').popup();
        ...
        let fxconfig = { height: 700
            ...
            , columns: [{ name: "query count", size: 330
                ...
                ,{ name: "min time (ms)", size: 130
                    , thAttributes: "id='minTime_OrderIcon' style='cursor : pointer' onClick='qan._orderSummaryColumn(\"minTime\");'"}
                ,{ name: "max time (ms)", size: 130
                    ,thAttributes: "id='maxTime_OrderIcon' style='cursor : pointer' onClick='qan._orderSummaryColumn(\"maxTime\");'"}
        ...
        for (let i = 0 ; i < anas.length; i++) {
            ...
            //
            let th = 0;
            let thval = document.getElementById(this.SUMMARY_TRESHOLD);
            if (thval && parseInt(thval.value) > 0 ) {
                if (dta.avgTime < parseInt(thval.value)) {
                    continue;
                }
            }
            ...
            if (applyQueryFilter && applyQueryFilter == true) {
                ...
                let tresholdpos = queryFilter.indexOf("th:");
                if (dbpos == -1) {
                    if (dta.query.toLowerCase().indexOf(filter.toLowerCase()) == -1) {
                        continue;
                    }
            ...
            fxrow += "</div><br/>";
            if (dta.count == 1) {
                fxrow += "<div style='font-size: 0.8em'><b>" + this._formatDate(dta.minLogTime, false) + "</b></div></td>"
            } else {
                fxrow += "<div style='font-size: 0.8em'><b>" + this._formatDate(dta.minLogTime, false) + "</b> to <b>" + this._formatDate(dta.maxLogTime, false) + "</b></div></td>"
            }
...
//            fxrow += "<td>" + dta.logdate + "</td>";
            fxrow += "<td>" + this._formatDate( dta.logdate, true ) + "</td>";
            let dbn = "__";
            if (dta.dbname && dta.dbname != "" && dta.dbname !== "<none>") {
                dbn = dta.dbname;
            }
    ...
    _formatDate(ts, showMillis) {
        ...
        res += d.getSeconds();
        if (showMillis && showMillis === true) {
            res += "."
            if (d.getMilliseconds() < 10) {res += "00"}
            if (d.getMilliseconds() > 9 && d.getMilliseconds() < 100) {res += "0"}
...
//        this._buildQueryLog(-1, true);
    ...
    _analyseQueriesPerMinuteUnit(index) {
        ...
        // graph2d.on("click", this._onTimeLineSelect);
    }
    _isNewEntry(row) {
...
//        console.log("aRecord[6]; " + aRecord[6]);
    ...
    _parseAuraJSONLine(aRecord) {
        // console.log("_parseJSONLine: " + JSON.stringify(aRecord));
        let rec = aRecord;
        let qr = this._createQRecord();
        qr.startOnly = (rec.event == "start");
        qr.error = (rec.event == "fail");
    ...
    _parseBoltLineV4Plus(aRecord) {
        ...
        if (qr.startOnly && qr.startOnly == true) {
            ...
            if (field0.indexOf("transaction id:") > -1) {
                ...
                let tIndex = this._getFieldIndex(field0fields,"-") ;
                let msIndex = this._getFieldIndex(field0fields, "ms:");
                if (tIndex >= msIndex) {
                    // now we can use the ms index
                    queryMillisIndex = this._getFieldIndex(field0fields,"ms:") -1 ;
                } else {
...
//        console.log("queryMillis on index " + queryMillisIndex + " value>>" + field0fields[queryMillisIndex] + "<<:" +  parseInt(field0fields[queryMillisIndex]));
        ...
        // An embedded line can also have a query Id
        qr.qid = -1;
        // console.log(field0fields);
        if (qr.startOnly === true) {
            qr.qid = this._getQueryId(field0fields[6]);
    ...
    _getValueField0(s, name) {
        ...
        if (namePos > -1) {
            ...
            if (left == true) {
                ...
                if (name == " B -" ) {
                    let fp = s.substring(0, namePos).trim();
                    let colonPos = fp.lastIndexOf(" ")
                    if (colonPos > -1) {
                        let valString = fp.substring(colonPos + 1)
                        r = parseInt(valString.trim());
                    }
                } else if (name == "page hits") {
