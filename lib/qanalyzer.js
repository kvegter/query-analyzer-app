...
class QueryAnalyzer {
    ...
    analyseFile(aFileString) {
        ...
        appSummary.innerHTML = this._buildSummary(this.ana);
    ...
    _analyseLog(logLines) {
        ...
        for (let ri = 0; ri < logLines.length; ri++ ) {
            ...
            if (this._isEndEntry(row)) {
                ...
                curLine = "";
                if (record.length == 8 || record.length == 5 || record.length == 3) {
                    ...
                    if (map.has(qrec.query)) {
                        qs = map.get(qrec.query);
                    } else {
                        qs.query = qrec.query;
                    }
                    ...
                    map.set(qrec.query, qs);
                } else {
                    console.log("WARNING unexpected record length " + record.length);
                    console.log(record);
                }
            } else {
                // multiline log entry
    ...
    _buildSummary(anas) {
        ...
        // table header
        contents+= "<thead><tr>";
        contents+= "<th class='hover'>Query Count<div class='tooltipth'>Filter: show only the query in query log<br/>Highlight: highlight the query in query log</div></th>"
        contents+= "<th>Avg Time</th>"
        contents+= "<th>Min Time</th>"
        contents+= "<th>Max Time</th>"
        contents+= "<th>Avg CPU</th>"
        contents+= "<th>Max Planning</th>"
        contents+= "<th>Avg Waiting</th>"
        contents+= "<th>Cache hits %</th>"
        contents+= "<th class='hover'>Avg Mem (K)<div class='tooltipth'>The cumulative heap memory usage of a query in KB</div></th>"
        ...
        for (let i = 0 ; i < anas.length; i++) {
            ...
            contents += "<td><button class='compact ui teal basic button' onClick='qan._showFilteredLog(" + i + ", false);'>Filter</button>";
            contents += "<button class='compact ui teal basic button' onClick='qan._showFilteredLog(" + i + ", true);'>Highlight</button>";
            contents += "<button class='compact ui teal basic button' onClick='qan._showQueryTimeline(" + i + ");'>Timeline</button><br/>";
            if (dta.count == 1) {
    ...
    _buildQueryLog(index, showAll){
        ...
        if (applyFilter) {
            if( showall) {
                contents += "Use tab/shift-tab to navigate the marked query lines, marked with filter: <p>" + this._formatCypher(bQueryFilter);

            } else {
                contents += "Use tab/shift-tab to navigate the query lines, showing only queries with filter: <p>" + this._formatCypher(bQueryFilter);
            }
        } else {
        ...
        for (let i = 0; i < this.log.length; i++) {
            ...
            if (applyFilter) {
                if (dta.query.indexOf(bQueryFilter) > -1) {
                    if (rowbackground) {
            ...
            if (isFiltered) {
                contents += "<td><div style='width: 15400px; word-wrap : normal'>see above</div></td>";
            } else {
                contents += "<td><div style='width: 15400px; word-wrap : normal'>" + this._formatCypher(dta.query) + "</div></td>";
            }
            contents += "</tr>";
            rowbackground = !rowbackground;
        }
    ...
    _formatCypher(aText) {
        // split text in words
        let keywords = new Array("<>",">=","<=","<",">","!=","=","not","unique", "distinct","match","set","then","using","optional","index","remove","merge","on","create","delete","detach","return","from","where","with","skip","limit","call","yield","foreach","case","when","else","end","or","as","and","union", "all", "order", "by", "unwind", "desc", "asc" );
        let res = "";
//        let hlcolor = "#919A78"; #00ff00, #63B345
    ...
    _parseQueryPart(queryField) {
        ...
        return {neoUser: neoUser, query: query, params: params , txmeta: txmeta};
    }
