...
class QueryAnalyzer {
    ...
    //         // tricky if we use 'current' in the constructor than you get the 'current' snapshot in time for 'this' which is not completely filled
    //         // Putting it here than it is save
    //         console.log(" REGISTERING THE FILTER INPUT KEY");
    //         current.onFilterChange(event);
    //     });
    // }
    _prepareSummarytab() {
        ...
        let html = "<div id='"+ this.SUMMARY_FILTER_KEY+ "' class='ui fluid left icon input'><input  id='"+ this.FILTER_INPUT_KEY + "' type='text' placeholder='Hit enter to Filter the queries containing this text. When empty all the distinct queries will be shown. '/><i class='filter icon'></i></div>";
    ...
    _initUnFinished() {
        ...
        if (this.onlyStarted.size > 0 ) {
            let headerhtml = "<div class='ui segment'>There are " + this.onlyStarted.size
                           + " unfinished queries found. The highest query id in this log is "
    ...
    _initQueryLogImpl(withFilters){
        ...
        html += "<button class='compact ui teal basic button' onClick='qan._showLogLoading()'>Show Query Log</button> Note this can take some time when there a lot queries in the log";
        html +="</div>";
        ...
        if (record.length == 9 || record.length == 8 || record.length == 5 || record.length == 3 || record.length == 7 || record.length > 9 ) {
            ...
            if (qrec.startOnly != undefined && qrec.startOnly === false) {
                ...
                if (this.onlyStarted.has(qrec.qid)) {
                    // console.log(" removing entry for " + qrec.qid);
