...
class QueryAnalyzer {
    ...
    async _saveQueryLogSettings() {
        ...
        if (nac.neodb.isCluster) {
            ...
            this._saveQueryLogSettingsImpl("", this.nac.driver);
        }
    }
    ...
    async _saveQueryLogSettingsImpl(aKey, aDriver) {
        ...
        // console.log(config);
        if (nac.neodb.majorVersion > 3) {
            let queryLogVal =  this._getRadioGroupValue(document.getElementsByName("r_queryLog" + key));
            if (config.queryLog.value !== queryLogVal) {
                updateStatements.push("call dbms.setConfigValue('"+ config.queryLog.property+"','"+ queryLogVal +"')");
            }
        } else {
            let queryLogVal =  "" + document.getElementById("queryLog" + key).checked;
            if (config.queryLog.value !== queryLogVal) {
                updateStatements.push("call dbms.setConfigValue('"+ config.queryLog.property+"','"+ queryLogVal +"')");
            }
        }
        ...
        if (nac.neoConnection && nac.neoConnection == true && nac.neodb.apocAvailable == true) {
            ...
            try {
                if (nac.driverMap && nac.driverMap.size > 0) {
                    for (let ii = 0; ii < nac.driverMap.size; ii++) {
                        try {
                            ...
                            //
                            // console.log("-------- key " + key);
                            // let trs = await nac.runQuery(session , "call dbms.listConfig() yield name, value where name starts with 'dbms.directories.' or name starts with 'dbms.logs.query.' \n" +
                            //     "return name, value", {});
                            // trs.records.forEach(function(record) {
                            //    console.log("name: " + record.get("name") + " value: " + record.get("value"));
                            // });
                            msg.innerHTML = "\n" +
                ...
                if (ermsg.indexOf("NoSuchFileException",0) > -1 && ermsg.indexOf("apoc.log.stream", 0) > -1) {
                    ermsg = "There is no query.log file available on the server, try to switch Query logging on in the Query Log Settings tab";
                }
                msg.innerHTML = ermsg;
            }
