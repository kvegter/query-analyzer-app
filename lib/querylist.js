    ...
    prepareTrace() {
        ...
        if (nac.neodb.majorVersion > 3) {
            for (let i = 0; i < nac.neodb.databases.length; i++) {
                ...
                if (dbinfo.currentStatus == "online" && dbinfo.name != "system" && dbinfo.name != nac.neodb.fabricDatabaseName) {
...
//            console.log("a_ " + a_ + " b_ " + b_ );
    ...
    async currentQueries(appContainer, dbname) {
        ...
        if (nac.neodb.majorVersion > 3) {
            query = "call dbms.listQueries() YIELD queryId, username, metaData, query, parameters, planner, runtime, indexes, startTime, protocol, clientAddress, requestUri, status, resourceInformation, activeLockCount, elapsedTimeMillis, cpuTimeMillis, waitTimeMillis, idleTimeMillis, allocatedBytes, pageHits, pageFaults, connectionId, database WHERE database = '" + dbname + "' RETURN *";
        }
        //console.log("session ");
        //console.log(session);
        let rs = await nac.runQuery(session, query);
    ...
    async showQueryPlan(aQuery, parameters, dbname) {
        ...
        try {
            ...
            qpq.innerHTML = '<p>The plan for this query could not be retrieved. This can be caused by analyzing an old log file which' +
                ' uses unsupported functions.</p> <br/><br/><div style="color: red">' + err.message.split(/\r?\n/)[0] + "</div>"
