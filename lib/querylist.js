        ...
        if (nac.neoConnection && nac.neoConnection == true) {
            ...
            if (nac.neodb.majorVersion > 3 || (nac.neodb.majorVersion == 3 && nac.neodb.minorVersion > 4)) {
                if (!nac.neodb.isAura) {
                    ...
                    this.elTraceMessage.innerHTML = "<div class='ui segment'> This functionality is not available on Aura, use 'Analyse Current Log' to get the latest query information</segment>";
                }
            }
        }
        return prms;
...
//            console.log("a_ " + a_ + " b_ " + b_ );
    ...
    _getQueriesSegment(container, databasename, intab, act) {
        let html = "";
        if (intab && intab == true) {
            html += '<div align="center" style="font-size: .95em" class="ui bottom attached ' + act + 'tab segment" data-tab="cqt_' + databasename + '">'+
    ...
    _getTraceQueriesSegment(container, databasename, intab, act) {
        ...
        let html = "";
        if (intab && intab == true) {
            html += '<div align="center" style="font-size: .95em"  class="ui bottom attached ' + act + 'tab segment" data-tab="tqt_' + databasename + '">'+
                '<div class="ui raised segment" ' +
    ...
    async currentQueries(appContainer, dbname) {
        ...
        for (let i = 0 ; i < records.length; i++) {
            ...
            contents += "<td><div class='ui inverted green icon buttons'><button class='ui button' title='Explain' onClick='querylistapp._explainCurrentQuery(" + i + ", \"" + dbname + "\");'><i class='vertically flipped sitemap icon'></i></button></div>";
...
//        console.log("showQueryModal for dbname " + dbname);
               ...
               "<td valign='top' >" +
                 "<div class='wrapper'>" +
                   ...
                   "<div id='minimapWrapper' style='position: absolute; margin: 5px; border: 1px solid #ddd; overflow: hidden; background-color: #FFF; z-index: 90;' class='minimapWrapperIdle'>" +
                    ...
                    "</div>" +
                   "</div>" +
                  "<table><tr valign='top'><td class='ui segment'><div id='queryTxt'  style='width: 1150px; height: 100px; overflow: auto'></div></td></tr>" +
                 "</table>" +
               "</td>" +
    ...
    async showQueryPlan(aQuery, parameters, dbname, srv) {
        ...
        try {
            ...
            qpq.innerHTML = '<p>The plan for this query could not be retrieved. This can be caused by analyzing an old log file which' +
                ' uses unsupported functions.</p> <br/><br/><div style="color: #ff0000">' + err.message.split(/\r?\n/)[0] + "</div>"
...
//style='color : " + this._getColorPerOperatorType(step) +"'
            //console.log(step);
            let r = "";
            r = r + "<table style='width: 450px'><tr><td><h3 class='ui header'><i class='dot circle outline icon'></i><div class='content'>" + step.operatorType + "</div></h3></td>" +
