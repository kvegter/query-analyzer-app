...
//        console.log(rs);
        ...
        if (srv !== "") {
            this._setTraceRecords(dbn + "@" + srv, records);
            this._buildTraceTable(container, dbn + "@" + srv, records);
        } else {
        ...
        let config = { subContainerWidth: 1650
            ...
            , columns: [{ name: "&nbsp;", size: 100}
                        ...
                        ,{ name: "last Invocation", size: 150
                              , thAttributes: "id='invocationSummary.invocationCount_OrderIconTrace_" + dbn + "' style='cursor : pointer' onClick='querylistapp._orderTraceColumn(\"invocationSummary.invocationCount_OrderIconTrace_" + dbn + "\",\"" + dbn  +  "\");'"}
                        ,{ name: "query", size: 650}
...
//        console.log("add totals start + " + dta.invocations.length);
    ...
    _orderTraceColumn(property, dbname) {
        ...
        dta.sort(function(a,b) {
            let prop = "lastInvocation"
            let a_ =  a.lastInvocation;// current._resolve(prop, a);
            let b_ =  b.lastInvocation;//current._resolve(prop, b);
            if (orderASC) {
                if (a_ < b_) return -1;
                if (a_ > b_) return 1;
            } else {
        ...
        // count_OrderIcon
        //let elmId = property + "_OrderIconTrace_" + dbname;
        let elmId = property ;
        //console.log("elm id " + elmId);
        let iconElm = document.getElementById(elmId);
