...
//        console.log("after show")
    ...
    async getNeo4jVersion(dbName, pw) {
        ...
        if (this.neodb.majorVersion > 4) {
            queryseconoff = "show procedures yield name where name contains '.showCurrentUser' return count(name) as cnt";
        }
       ...
       //if (this.neodb.isAdmin == true) {
        ...
        if (nac.neodb.majorVersion > 4) {
            apc = "show procedures yield name where name starts with 'apoc.' return count(*) as cnt";
        }
    ...
    async runQueryBase(session, qry, params) {
        ...
        let records = [];
        let results;
        let prom = session.run(qry, params, {metadata : { app : "QueryLogAnalyzer_v1.0.5", type: "user-direct"}});
        //await prom;
